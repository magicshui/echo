...
func TestFunctionLiterParsing(t *testing.T) {
	input := `fn(x,y){x+y}`
	l := lexer.New(input)
	p := New(l)
...
func testIdentifier(t *testing.T, exp ast.Expression, value string) bool {
	...
	if !ok {
		t.Errorf("il not *ast.Identifier. got=%T", exp)
	}

	if ident.Value != value {
		t.Errorf("ident.Value not %d. got=%d", value, ident.Value)
		return false
	}

	if ident.TokenLiteral() != value {
		t.Errorf("ident.TokenLiteral not %d. got=%s", value, ident.TokenLiteral())
		return false
	}
